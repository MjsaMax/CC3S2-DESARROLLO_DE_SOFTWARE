...
class TestIMDbDatabase:
    ...
    def test_movie_reviews_failure(self, mock_get):
        ...
        resultado = imdb.movie_reviews("NoHayReview")

        assert resultado == {}
        mock_get.assert_called_once_with(
            "https://imdb-api.com/API/Reviews/fake_api_key/NoHayReview"
        )
    ...
    def test_movie_ratings_failure(self, mock_get):
        ...
        mock_get.return_value = mock_response
        imdb = IMDb(apikey="fake_api_key")
        resultado = imdb.movie_ratings("IDratingfalla")
        assert resultado == {}
        mock_get.assert_called_once_with(
            "https://imdb-api.com/API/Ratings/fake_api_key/IDratingfalla"
        )
