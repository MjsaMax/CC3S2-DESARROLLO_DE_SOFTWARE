...
def test_update_counter_failure(client):

    response = client.put("/counters/non_existent_counter")
    assert response.status_code == HTTPStatus.NOT_FOUND
    data = response.get_json()
    assert data["message"] == "El contador 'non_existent_counter' no existe"
...
def test_delete_counter_failure(client):

    response = client.delete("/counters/non_existent_counter")
    assert response.status_code == HTTPStatus.NOT_FOUND
    data = response.get_json()
    assert data["message"] == "El contador 'non_existent_counter' no existe"
